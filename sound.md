...
ゲームボーイはPSG(Progrmmable Sound Generator)と呼ばれる音源を持っており、PSGは4つのチャネルで構成されています。(PCMチャネルはありません)
...
チャンネルは、それぞれ、まず `0..15` の音量を出力し、それがチャンネルの持つDACによって `-1..1` の範囲に変換されます。

最後に、これら4つの値がミキシングされ、左右のスピーカーに出力されます。(つまり、最終出力は `-4..4` の範囲になります)
...
このレジスタ(`NR12`)のbit3-7をすべて0にすると、DACがオフになり(したがってチャンネルもオフになり)、ポップノイズが発生する可能性があります。
